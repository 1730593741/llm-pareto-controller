...
REM ==========================================
REM DWTA 大规模静态场景（12x24）多 seed 运行脚本
REM - 保持 env-driven LLM（不写任何真实 key）
REM - 输出目录兼容 collect_summaries.py
REM ==========================================
set SEEDS=42 100 2024 1234 9999
set PYTHON_CMD=python
set TEMP_CFG_DIR=tmp_seed_configs_dwta_large
set RUNS_ROOT=experiments\runs\dwta_large_results
set CFG_DWTA_BASE=experiments\configs\dwta_large_static.yaml

if not exist "%TEMP_CFG_DIR%" mkdir "%TEMP_CFG_DIR%"
if not exist "%RUNS_ROOT%" mkdir "%RUNS_ROOT%"

echo ==========================================
echo [INFO] 开始批量运行 DWTA LARGE STATIC 多 seed 实验
...
for %%s in (%SEEDS%) do (
    ...
    set BASE_CFG=%TEMP_CFG_DIR%\baseline_nsga2_dwta_large_seed_%%s.yaml
    ...
    set RULE_CFG=%TEMP_CFG_DIR%\rule_control_dwta_large_seed_%%s.yaml
    echo [2/4] 生成 Rule 配置...
    ...
    set MOCK_CFG=%TEMP_CFG_DIR%\mock_llm_dwta_large_seed_%%s.yaml
    echo [3/4] 生成 Mock 配置...
    ...
    set REAL_CFG=%TEMP_CFG_DIR%\real_llm_dwta_large_seed_%%s.yaml
    echo [4/4] 生成 Real 配置...
...
echo.
echo [SUCCESS] DWTA LARGE STATIC 多 seed 实验运行完成。
echo [INFO] 汇总命令示例:
echo        python collect_summaries.py experiments/runs/dwta_large_results experiments/exports/dwta_large_results
pause
