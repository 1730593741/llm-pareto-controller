...
def test_problem_config_accepts_legacy_munitions_key() -> None:
    cfg = ProblemConfig.model_validate(
        {
            "problem_type": "dwta",
            "munitions": [{"id": "m1", "max_range": 10.0, "flight_speed": 2.0, "lethality": 1.5}],
...
def test_scenario_builder_rejects_unknown_munition_reference() -> None:
    with pytest.raises(ValueError, match="unknown munition_type_id"):
        build_scenario_matrices(
            ...
            targets=[Target(id="t1", x=1.0, y=1.0, required_damage=1.0, time_window=(0.0, 1.0))],
        )
